...
<html>
<head>
    <title>Standard Invoice Template</title>
    <meta charset="UTF-8" />
...
<body class="tailwind-container">
    ...
    @foreach ($models as $model)
        ...
        <?php
            ...
            // Modeldeki tüm ürünleri döngüye alın
            foreach ($model->articles as $article) {
                ...
                // Eğer TVA oranı dizi içinde yoksa, bu oranı ve hesaplamalarını diziye ekleyin
        ...
        <section class="h-full flex flex-col justify-between items-center">
            <header>
                <div class="flex justify-between items-center">
                    <div>
                        <svg version="1.0" width="50px"  viewBox="0 0 604.913 751.988" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg">
            ...
            <main>
                <div class="flex justify-between mb-8">
                    <div>
                        ...
                        <p>{{ $model->client->last_name }}</p>
                        @foreach ($model->client->ClientAddress as $ClientAddress)
                            @if($ClientAddress->billing_address)
                                <p>{{ $ClientAddress->address }}</p>
                                <p>{{ $ClientAddress->postal_code }}, {{ $ClientAddress->city }}</p>
                            @endif
                        @endforeach
                    </div>
                    <div>
                        <p><strong>N° Devis:</strong> {{ $model->getKey() }}</p>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr>
                            <th class="py-2 px-4 border border-gray-600 font-semibold">Id</th>
                            <th class="py-2 px-4 border border-gray-600 font-semibold">Désignation</th>
                            <th class="py-2 px-4 border border-gray-600 font-semibold">Quantité</th>
                            <th class="py-2 px-4 border border-gray-600 font-semibold">TVA</th>
                            <th class="py-2 px-4 border border-gray-600 font-semibold">HT</th>
                            <th class="py-2 px-4 border border-gray-600 font-semibold">TTC</th>
                            <th class="py-2 px-4 border border-gray-600 font-semibold">Total HT</th>
                            <th class="py-2 px-4 border border-gray-600 font-semibold">Total TTC</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($model->articles as $article)
                            <tr>
                                <td class="py-2 px-4 border border-gray-600">{{ $article->id }}</td>
                                <td class="py-2 px-4 border border-gray-600">{{ $article->designation }}</td>
                                <td class="py-2 px-4 border border-gray-600">{{ $article->pivot->quantity }}</td>
                                <td class="py-2 px-4 border border-gray-600">{{ $article->vat_rate }}</td>
                                <td class="py-2 px-4 border border-gray-600">{{ number_format($article->selling_price, 2) }} €</td>
                                <td class="py-2 px-4 border border-gray-600">{{ number_format($article->price_including_tax, 2) }} €</td>
                                <td class="py-2 px-4 border border-gray-600">{{ number_format($article->selling_price * $article->pivot->quantity, 2) }} €</td>
                                <td class="py-2 px-4 border border-gray-600">{{ number_format($article->price_including_tax * $article->pivot->quantity, 2) }} €</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
                <table class="w-full text-left border-collapse">
                    <tbody>
                        <tr>
                            <td class="py-2 px-4 text-right font-semibold w-3/4">Total HT:</td>
                            <td class="py-2 px-4 w-1/4">{{ number_format($model->total_ht, 2) }} €</td>
                        </tr>
                        @foreach ($grouped_vat as $vat_rate => $totals)
                            <tr>
                                <td class="py-2 px-4 text-right font-semibold w-3/4">TVA {{ $vat_rate }}%:</td>
                                <td class="py-2 px-4 w-1/4">{{ number_format($totals['total_tva'], 2) }} €</td>
                            </tr>
                        @endforeach
                        <tr>
                            <td class="py-2 px-4 text-right font-semibold w-3/4">Total TVA:</td>
                            <td class="py-2 px-4 w-1/4">{{ number_format($model->total_ttc - $model->total_ht, 2) }} €</td>
                        </tr>
                        <tr>
                            <td class="py-2 px-4 text-right font-semibold w-3/4">Total TTC:</td>
                            <td class="py-2 px-4 w-1/4">{{ number_format($model->total_ttc, 2) }} €</td>
                        </tr>
                    </tbody>
                </table>
            </main>
            <footer class="mt-8">
                <hr class="my-4">
                <p class="font-light text-sm">Sayfa : {{ $loop->iteration }}</p>
        </footer>
    </section>
@endforeach
</body>
